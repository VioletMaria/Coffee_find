...
class User:
    ...
    def validate_user(user):
        ...
        if not email_regex.match(user['email']):
            ...
            valid_email = connectToMySQL("coffee_find_db")
    ...
    def insert_user(cls,data):
        ...
        return connectToMySQL("coffee_find_db").query_db(query,data)

    @classmethod
    def get_by_email(cls,data):
        query = "SELECT * FROM users WHERE email = %(email)s"
        user_db = connectToMySQL("coffee_find_db").query_db(query,data)
        if len(user_db) < 1:
            return False
